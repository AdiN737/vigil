...
class HookProcessTests(unittest.TestCase):
    ...
    def command(self, state):
        ...
        return ([binary] if binary else [sys.executable, str(ROOT / 'app/vigil_hook_main.py')]) + [state, 'codex']
    ...
    def test_passive_lifecycle(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(json.loads(files[0].read_text())['provider'], 'codex')
        ...
        for decision in ('allow', 'deny'):
            with self.subTest(decision=decision), tempfile.TemporaryDirectory() as td:
                ...
                try:
                    ...
                    self.assertLess(record['tier'], 4, 'Answered approval remains actionable')
                    self.assertEqual(list((Path(td) / 'requests').glob('*.json')), [])
                finally:
                    if p.poll() is None: p.kill(); p.communicate()

    def test_timeout_defers_to_codex(self):
        with tempfile.TemporaryDirectory() as td:
